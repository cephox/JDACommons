...
public class EmoteReactionListener extends ListenerAdapter {
    ...
    private long messageId, textChannelId, emoteId;
    private ReactionState trigger;

    public EmoteReactionListener(long emoteId, long messageId, ReactionState trigger) {
    ...
    public void onGuildMessageReactionAdd(@Nonnull GuildMessageReactionAddEvent event) {
        try {
            if (event.getReactionEmote().getEmote().getIdLong() == emoteId && !event.getMember().getUser().equals(event.getJDA().getSelfUser())) {
    ...
    public void onGuildMemberLeave(@Nonnull GuildMemberLeaveEvent event) {
        try {
            ...
            channel.removeReactionById(messageId, guild.getEmoteById(emoteId), member.getUser()).queue();
        } catch (Exception ex) {
        }
    }
